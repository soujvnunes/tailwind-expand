...
describe('extractor', () => {
  it('extracts simple @expand block', () => {
    const result = extract(fixture('simple.css'));
  ...
  it('extracts nested modifiers', () => {
    const result = extract(fixture('nested.css'));
  ...
  it('extracts deeply nested modifiers', () => {
    const result = extract(fixture('deep.css'));
  ...
  it('merges multiple @expand blocks for same name', () => {
    const result = extract(fixture('merge.css'));
  ...
  it('allows namespace-only @expand (no @apply)', () => {
    const result = extract(fixture('namespace.css'));
  ...
  it('throws on non-CamelCase alias', () => {
    expect(() => extract(fixture('invalid-name.css'))).toThrow('CamelCase');
  });

  it('generates consistent hash for same content', () => {
    const result1 = extract(fixture('simple.css'));
    const result2 = extract(fixture('simple.css'));
